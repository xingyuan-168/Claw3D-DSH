...
Run these in order, then call `aios_finish_check` (CLI: `aios finish`) with the task facts (`memory_written` or `memory_not_needed`). The gate re-verifies the repository side itself.
...
6. **Architecture evidence** — only when this change affected `docs/architecture/*.json`: regenerate the artifact through the pinned Archify skill and update `.aios/artifacts/archify/index.json`. Small changes never force Archify.
7. **Git** — one Conventional Commit for the logical change, pushed to the task branch; `git status --porcelain` clean afterwards.
