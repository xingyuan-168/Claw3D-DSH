...
description: Isolate parallel or risky task work in a disposable Git worktree under .worktrees/ with DSH-native Agent/Subagent/Workflow/Agent Teams. Use when tasks would write overlapping paths in parallel, or when a long experiment should not touch the main worktree. Not needed for ordinary sequential work.
---

# Worktree protocol

DeepSeek Harness remains the execution authority for deciding when to use agents and how to split work. AIOS only keeps parallel writes isolated and registered.

## Prepare

- Create with `worktree_manage(action="prepare", name="task-slug")` (or `aios worktree prepare`). Worktrees live under `.worktrees/` and are registered so the hook treats them as disposable.
