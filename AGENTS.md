...
本仓库是 **DeepSeek Harness（DSH）的工程治理层**：无状态三 Gate（Code Start / Frontend Approval / Finish）只回答"允许/不允许及为什么"，不指导 DSH 怎么做专业工作。以下十条为硬治理规则，违反任何一条都不得继续。

> **最高优先级规则：任何 AI OS 新功能开发前必须先检查 DSH 是否已经提供该执行能力；DSH 已有能力禁止重复实现。**

## 十条宪法

1. **不改变 DeepSeek Harness 原生工程方式**：AIOS 只治理"能否做、何时做、做完留什么"；Agent、Subagent、Workflow、Agent Team、Task、Session、Tool、Approval、Sandbox、Workspace、Model 都归 DSH，AIOS 不重新实现。
2. **正式编码前必须通过 Code Start Gate**：GitHub remote 存在可达；开源调研分层完成并记录 Decision；仓库无复制式脏乱、无未解决冲突。无 GitHub 时允许读 input/、分析、调研、规划、写文档，禁止正式 src/ 实现。客观边界由 Hook/Runtime 自动强制，`aios check` 可主动预看阻塞原因。
...
7. **并行任务用 Worktree 隔离**：DSH 原生 Agent/Subagent 各自进入 .worktrees/ 下的登记 Worktree，不写主工作区与其他 Worktree；完成后 review、merge、及时 cleanup，不留孤儿 Worktree。
