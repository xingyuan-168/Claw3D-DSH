...
评估器位于 `src/ai_engineering_os/core/gates.py`，同输入同输出，不持有隐藏状态。
...
默认验证 ≤5 项：目标测试、ruff、`git diff --check`、仓库卫生（`aios check`）、必要时 pyright。每个逻辑变更还须通过仓库 Secret Scan（detect-secrets 只扫本次修改，封装在 `scripts/secret_scan_incremental.py`）。
...
- 已初始化项目的 apply_patch / 重定向目标经 `aios authorize-hook` 内核裁决（仅主工作区）。
