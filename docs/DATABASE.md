...
SQLite 只保存运行状态与可重建索引；Markdown/Git 保存事实正文。数据库位于 `.aios/state/state.db`，整个 `.aios/` 被 Git 忽略。
...
- 预重构数据库（表集、列集或 checksum 不匹配）自动导出为 `.aios/state/backups/<stem>-legacy-<stamp>.db`（附 SHA-256 sidecar）后从头重建；不做数据迁移（旧表全部被取代）。
