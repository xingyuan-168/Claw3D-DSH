...
Governance commands: init, check, finish, memory, worktree, mcp (deprecated),
doctor. The CLI answers "allowed / not allowed and why" and manages the
light runtime state; it never tells the DeepSeek Harness how to do
professional work.
"""
...
from ai_engineering_os.infrastructure.memory import (
    MemoryEntry,
    MemoryStore,
    MemoryStoreError,
)
...
app = typer.Typer(
    name="aios",
    help="Local, auditable engineering governance layer for DeepSeek Harness.",
...
def _project_database(project_root: Path) -> tuple[Any, Database]:
    ...
    database = Database(config.root / ".aios" / "state" / "state.db")
    database.migrate()
...
def mcp_command() -> None:
    ...
    from ai_engineering_os.cli.mcp_server import run_server
...
def worktree_prepare_command(
    ...
    dsh_session_id: Annotated[str | None, typer.Option("--dsh-session-id")] = None,
    dsh_agent_id: Annotated[str | None, typer.Option("--dsh-agent-id")] = None,
    project_root: Annotated[Path, typer.Option("--project-root")] = Path("."),
    json_output: Annotated[bool, typer.Option("--json")] = False,
) -> None:
