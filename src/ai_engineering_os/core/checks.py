...
as "the docs are consistent with the change" remain the DeepSeek Harness's duty and are
...
from ai_engineering_os.core.gates import GateFinding, disposable_findings, hygiene_findings
from ai_engineering_os.infrastructure.memory import CANDIDATE_DIRECTORY
...
def _formal_dirty_paths(root: Path, git: GitRunner) -> list[str]:
    ...
    from ai_engineering_os.infrastructure.config import load_project_config
...
def _code_start_recheck_findings(
    ...
    *,
    ...
    from ai_engineering_os.core.gates import evaluate_code_start
