"""Disposable Git worktree lifecycle for DeepSeek Harness agents (ADR-0016).
...
database so the DSH governance plugin can treat them as disposable
areas; cleanup unregisters them. There is no coordination, lease, or DAG
machinery: the lead DSH session owns merge decisions; AIOS only keeps
...
from ai_engineering_os.domain.ids import new_id
...
class WorktreeManager:
    ...
    def prepare(
        ...
        *,
        ...
        """Create a disposable worktree and register it for path checks.

        DSH session/agent ids are recorded for tracing only (V4 spec section
        8); DSH remains the sole source of task and approval state.
        """
            ...
            try:
                ...
                connection.execute(
                    ...
                    INSERT INTO worktrees(
                        id, name, path, branch, target_branch, dsh_session_id,
                        dsh_agent_id, disposable, status, created_at, updated_at
                    ) VALUES (?, ?, ?, ?, ?, ?, ?, 1, 'active', ?, ?)
                    """,
                    (
                        record_id,
    ...
    def finish(self, *, name: str) -> WorktreeRecord:
        ...
        "ready" is not "merged": the lead session (DSH or the user)
...
def _record_from_row(row: sqlite3.Row) -> WorktreeRecord:
    return WorktreeRecord(
        ...
        dsh_session_id=str(row["dsh_session_id"]) if row["dsh_session_id"] is not None else None,
        dsh_agent_id=str(row["dsh_agent_id"]) if row["dsh_agent_id"] is not None else None,
